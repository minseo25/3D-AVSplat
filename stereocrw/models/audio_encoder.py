...
class AudioEncoder(nn.Module):
    ...
    def wav2stft(self, audio):
        """
            audio: (N, K, 1, L)  # batch of K patches
            Returns: (N, K, 2, time-1, freq-1)
        """
        # 1) remove singleton "channel=1"
        audio = audio.squeeze(-2)        # now (N, K, L)
        # 2) apply Spectrogram (power=None) → complex64, shape = (N, K, freq, time)
        spec_complex = self.trans(audio)
        # 3) convert complex to real/imag pair → (N, K, freq, time, 2)
        spec_real_imag = torch.view_as_real(spec_complex)
        # 4) permute dims so that real/imag is in the "channel" slot:
        #    (N, K, freq, time, 2) → (N, K, 2, time, freq)
        spec = spec_real_imag.permute(0, 1, 4, 3, 2)
        # 5) crop off last freq/time bin
        spec = spec[..., :-1, :-1]
        # final shape = (N, K, 2, time-1, freq-1)
        return spec
...
class WaveNet(AudioEncoder):
    ...
    def __init__(self, args, pr, device=None, backbone=None):
        ...
        if backbone in ['resnet9']:
            ...
            # 1) instantiate a ResNet-9 (i.e. 4 stages of BasicBlock with [1,1,1,1])
            model = ResNet(BasicBlock, [1, 1, 1, 1])
            
            # 2) swap out the first conv to accept 2 channels
            model.conv1 = torch.nn.Conv2d(
                in_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False
            )
            
            # 3) swap out the final FC so it outputs pr.feat_dim
            ...
            # 1) instantiate a ResNet-18
            model = ResNet(BasicBlock, [2, 2, 2, 2])
            
            # 2) replace the first conv for 2-channel input
            model.conv1 = torch.nn.Conv2d(
                in_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False
            )
            
            # 3) replace final FC to output pr.feat_dim
            model.fc = nn.Linear(model.fc.in_features, pr.feat_dim)
            self.net = model
