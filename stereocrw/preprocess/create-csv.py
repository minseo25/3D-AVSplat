...
def create_list_for_video(args, name, video_list):
    ...
    for video in tqdm(new_video_list):
        rel_path = os.path.join('./stereocrw/preprocess', os.path.relpath(video, 'stereocrw/preprocess'))
        ...
        if name == 'train':
            sample_list.append({'path': rel_path})
        elif name == 'val':
...
def main(args):
    ...
    read_path = f'stereocrw/preprocess/ProcessedData/{args.dataset_name}'
    split_path = f'stereocrw/preprocess/data-split/{args.dataset_name}'
    if args.type != '':
